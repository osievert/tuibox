...
typedef struct ui_t
{
    ...
    int            mode, viewport_w, viewport_h, frame_drawn, drawing;
} ui_t;
 ...
 *   for mouse support.
 */
void _ui_new(int s, int mode, int w, int h, ui_t* u)
{
    struct termios raw;
...
int _ui_visible_strlen(char* s);
void _ui_print_clipped(char* s, int start, int end);

void _ui_draw_one_inline(ui_box_t* tmp, ui_t* u)
{
    char *buf, *tok;
    int n = 0;
    int start_x, end_x;
